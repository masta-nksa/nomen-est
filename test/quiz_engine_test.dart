...
void main() {
  ...
  List<CandidateStats> plainCandidates(int count) => [
        for (var i = 1; i <= count; i++) CandidateStats(studentId: i, box: 1, wrong: 0),
      ];

  group('question building', () {
    test('the asked student is always among the options', () {
      final engine = engineWith(plainCandidates(10));
      for (var i = 0; i < 30; i++) {
        final question = engine.next()!;
        expect(question.optionIds, contains(question.studentId));
      }
    });
    ...
    test('caps options at the number of students in a small class', () {
  ...
  group('confusion-based distractors', () {
    test('prefers the students actually confused with the target', () {
      final engine = engineWith(
        [
          const CandidateStats(studentId: 1, box: 1, wrong: 0, confusedWith: {7: 5, 8: 4}),
          for (var i = 2; i <= 20; i++) CandidateStats(studentId: i, box: 1, wrong: 0),
        ],
        settings: const QuizSettings(optionCount: 3, distractors: DistractorStrategy.confusion),
      );

      // Ask until student 1 comes up, then check who they are pitted against.
      for (var i = 0; i < 100; i++) {
        final question = engine.next()!;
        if (question.studentId == 1) {
          expect(question.optionIds, containsAll([1, 7, 8]));
          return;
        }
      }
      fail('student 1 was never asked about');
    });
  ...
  group('same-initial distractors', () {
    test('pulls in students sharing the first letter', () {
      final engine = engineWith(
        [
          const CandidateStats(studentId: 1, box: 1, wrong: 0, initial: 'M'),
          const CandidateStats(studentId: 2, box: 1, wrong: 0, initial: 'M'),
          const CandidateStats(studentId: 3, box: 1, wrong: 0, initial: 'M'),
          for (var i = 4; i <= 12; i++) CandidateStats(studentId: i, box: 1, wrong: 0, initial: 'Z'),
        ],
  ...
  group('student selection', () {
    test('shows everyone once before repeating', () {
      ...
      final asked = {for (var i = 0; i < 8; i++) engine.next()!.studentId};
      expect(asked, hasLength(8));
    });

    test('does not ask the same student twice in a row', () {
      final engine = engineWith(plainCandidates(5));
      var previous = engine.next()!.studentId;
      for (var i = 0; i < 40; i++) {
        final current = engine.next()!.studentId;
        expect(current, isNot(previous));
        previous = current;
      }
    });

    test('weights students in low Leitner boxes more heavily', () {
      final engine = engineWith([
        const CandidateStats(studentId: 1, box: 1, wrong: 10),
        for (var i = 2; i <= 6; i++) CandidateStats(studentId: i, box: 5, wrong: 0),
      ]);
      ...
      var struggling = 0;
      for (var i = 0; i < 120; i++) {
        if (engine.next()!.studentId == 1) struggling++;
      }
  ...
  group('Leitner movement', () {
    test('a correct answer moves up one box, capped at 5', () {
      final engine = engineWith([
        const CandidateStats(studentId: 1, box: 5, wrong: 0),
        const CandidateStats(studentId: 2, box: 1, wrong: 0),
      ]);
      engine.applyAnswer(studentId: 1, correct: true);
      engine.applyAnswer(studentId: 2, correct: true);
    ...
    test('a wrong answer drops two boxes, floored at 1', () {
      final engine = engineWith([
        const CandidateStats(studentId: 1, box: 5, wrong: 0),
        const CandidateStats(studentId: 2, box: 2, wrong: 0),
      ]);
      engine.applyAnswer(studentId: 1, correct: false);
      engine.applyAnswer(studentId: 2, correct: false);
